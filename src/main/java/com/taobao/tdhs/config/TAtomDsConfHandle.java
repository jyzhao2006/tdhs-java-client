...
public class TAtomDsConfHandle {
	...
	private TDHSClientImpl tdhsClient ; 
	/**
	 * 运行时配置
	 */
...
//			String port = runTimeConf.getPort();
			...
			InetSocketAddress iNetSocketAddress = new InetSocketAddress(ip,Integer.valueOf(port));
			//因为是epoll模型，所以连接数减少一些
			minPoolSize = minPoolSize/2;
			if(minPoolSize == 0)
			{
				minPoolSize = 1;
			}
			tdhsClient = new TDHSClientImpl(iNetSocketAddress, minPoolSize);
			tdhsClient.setDbName(runTimeConf.getDbName());
			initFalg = true;
		} finally {
			lock.unlock();
		}
	...
	public TDHSClient getTdhsClient() {
		return tdhsClient;
	}
